...
function printPostsAdmin(data){
    ...
    print+=`<table>
    <tr><th>Title</th><th>Image</th><th>Likes</th><th>Comments</th><th>Update</th><th>Delete</th></tr>
`
    for(let d of data){
        print+=`<tr>
            <td>${d.title}</td>
            ...
            <td>${d.likes}</td>
            <td>${d.comments}</td>
            <td><a href="#" class="updatePostGet" data-id="${d.id}">Update</a></td>
            <td><a href="#" class="deletePost" data-id="${d.id}">Delete</a></td>
            </tr>
            `;
    }
    ...
    $(".updatePostGet").click(updatePostGet)
}
...
function formAddPost(e,data=null){
    if(data!=null){
        var post=data[0]
    }
    e.preventDefault()
    $.ajax({
        url:BASE_URL+"/allAdmin/create",
        method:"get",
        dataType:"json",
        success:function(data){
            fillForm(data,post)
...
function fillForm(data,post=null){
    var print=``
    if(post==null){
    ...
    print+=`<form enctype="multipart/form-data">
        ...
        <p id="errorTitle"></p>
         ...
         <p id="errorPhotos"></p>
            <div>`
            for(let h of data.hashtags){
                print+=`<input type="checkbox" value="${h.idHashtag}" name="chbHash"><span> #${h.hashtag} </span>`
            }
         print+=`</div>`

               if(post==null){
                   print+=`<input type="button" value="Add post" id="addNewPost"/>`
                }
               else{
                   print+=`<input type="hidden" value="${post.post.id}" id="idPostHidden">`
                   print+=`<input type="button" value="Edit post" id="editPost"/>`
               }
    print+=`</form><p id="errorInsert"></p><div id="thumbnailPostAdmin"></div>
            <div id="imagesPostAdmin"></div>`

    $("#mainAdmin").html(print)
    if(post!=null){
        $("#title").val(post.post.title)
        $("#summernote").val(post.post.text)
        var tagovi=post.hashtag
        var allHash=$('input[name="chbHash"]')
        var all=[]
        for(let i=0;i<allHash.length;i++){
            all.push(allHash[i].value)
        }
        for(let i=0;i<tagovi.length;i++){
            if(!all.includes(tagovi[i].idHashtag)){
            $(`input[value="${tagovi[i].idHashtag}"]`).attr("checked",true)
            }
        }



        var img='<table><tr><th>Image</th><th>Delete</th></tr>'
        for(let d of post.images){
            img+=`<tr>
                     <td><img src="${BASE_URL+'/assets/images/'+d.src}" alt="${post.post.title}"/></td>
                     <td><a href="#" class="deletePostImage" data-id="${d.idImage}">Delete</a> </td>
                </tr>`
        }

        img+=`<table>`
        $("#imagesPostAdmin").html(img)
        $(".deletePostImage").click(deletePostImage)
